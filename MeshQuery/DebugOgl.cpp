...
bool init_gl_db() {
	...
	glBufferData(GL_ARRAY_BUFFER, 4 * MAX_APG_GL_DB_LINES * 14, NULL,
		GL_DYNAMIC_DRAW);
...
int add_gl_db_normal(float* n_xyz, float* pos_xyz, float scale,
	float* colour_rgba) {
	if (g_count_gl_db_lines >= MAX_APG_GL_DB_LINES) {
		fprintf(stderr, "ERROR: too many gl db lines\n");
		return -1;
...
int add_gl_db_frustum(float* ftl, float* ftr, float* fbl, float* fbr,
	float* ntl, float* ntr, float* nbl, float* nbr, float* colour_rgba) {
	int rid = -1;
	float start[3], end[3];
	start[0] = ftl[0];
	start[1] = ftl[1];
...
bool mod_gl_db_line(uint line_id, float* start_xyz, float* end_xyz,
	float* colour_rgba) {
